...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekBar = findViewById(R.id.sbMp3);
    ...
    //스레드 처리 메소드
     void threadProcess(){
         new Thread(){
             SimpleDateFormat format = new SimpleDateFormat("mm:ss");//분:초
             @Override
             public void run() {
                 ...
                 while(mPlayer.isPlaying()){
                     runOnUiThread(new Runnable() {
                         @Override
                         public void run() {
                             seekBar.setProgress(mPlayer.getCurrentPosition());//현재 재생위치 가져옴
                             tvTime.setText("진행시간 : "+format.format(mPlayer.getCurrentPosition()));
                         }
                     });
                     SystemClock.sleep(200);
                 }
